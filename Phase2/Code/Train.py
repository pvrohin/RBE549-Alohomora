...
def TrainOperation(TrainLabels, NumTrainSamples, ImageSize,
    ...
    for Epochs in tqdm(range(StartEpoch, NumEpochs)):
        ...
        # Calculate training accuracy for the entire training dataset for this epoch
        training_accuracy = total_correct / total_samples
        print(f"Epoch {Epochs}: Training Accuracy = {training_accuracy:.4f}")
        Writer.add_scalar('TrainingAccuracy', training_accuracy, Epochs)

        # Calculate training loss for the entire training dataset for this epoch
        training_loss = loss_per_epoch / NumIterationsPerEpoch
        print(f"Epoch {Epochs}: Training Loss = {training_loss:.4f}")
        Writer.add_scalar('TrainingLoss', training_loss, Epochs)
...
def main():
    ...
    Parser.add_argument('--LoadCheckPoint', type=int, default=1, help='Load Model from latest Checkpoint from CheckPointsPath?, Default:0')
